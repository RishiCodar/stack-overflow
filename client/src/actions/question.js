...
export const updateComment =(id,commentId, commentBody) => async (dispatch) =>{
  try {
      const {data} = await api.updateComment(id,commentId, commentBody);
      dispatch({type: 'UPDATE_CURRENT_COMMENT', payload: data})
      dispatch(fetchAllQuestions());
  } catch (error) {
      console.log(error);
  }
